...
impl ZeroDayStandalone {
    ...
    pub async fn run(&mut self) -> Result<Vec<Finding>> {
        ...
        println!("  {} {}",
            tc("◈", HISUI),
            tc("[Zero-Day Standalone] Initialising ML anomaly engine...", FUJI));
        ...
        if let Some(limit) = self.duration_limit {
            println!("  {} Duration limit: {}s",
                tc("▸", TSUYUKUSA),
                tc(&limit.as_secs().to_string(), FUJI));
        }
        println!();
        ...
        for url in &urls {
            ...
            print!("\r  {} [{}/{}] {:.0}%  |  {}  |  anomalies: {}  |  exploits: {}  |  remaining: {}   ",
                tc("▸", TSUYUKUSA),
                tc(&analyzed.to_string(), HISUI),
                tc(&total.to_string(), GIN),
                pct,
                tc(&truncate_url(url, 55), FUJI),
                tc(&self.total_anomalies.load(Ordering::Relaxed).to_string(), HISUI),
                tc(&self.confirmed_exploits.load(Ordering::Relaxed).to_string(), HISUI),
                tc(&remaining, GIN));
            ...
            for (pi, (label, payload)) in fuzz_payloads.iter().enumerate() {
                ...
                let result = match tokio::time::timeout(Duration::from_secs(5), fetch).await {
                    Ok(Ok(resp)) => {
                        if resp.status >= 500 {
                            errors_5xx += 1;
                            format!("{} {}[500]", tc(label_str, HISUI), tc("●", HISUI))
                        } else {
                            String::new()
                        }
                    }
                    ...
                    Err(_) => {
                        timeouts += 1;
                        format!("{} {}[TIMEOUT]", tc(label_str, HISUI), tc("◌", TSUYUKUSA))
                    }
                };
        ...
        println!("  {} Fuzz results for {} tests across {} targets",
            tc("╔═══", TSUYUKUSA),
            tc(&total_fuzz_tests.to_string(), FUJI),
            tc(&fuzz_targets.len().to_string(), GIN));
        println!("  {}  5xx errors:  {}  (server crashed or error page)",
            tc("║", fuzz_severity),
            tc(&errors_5xx.to_string(), if errors_5xx > 0 { HISUI } else { HISUI }));
        println!("  {}  Timeouts:     {}  (request hung / no response)",
            tc("║", fuzz_severity),
            tc(&timeouts.to_string(), if timeouts > 0 { HISUI } else { HISUI }));
        println!("  {}  Connection failures: {}  (connection refused / reset)",
            tc("║", fuzz_severity),
            tc(&crashed.to_string(), if crashed > 0 { SHU } else { HISUI }));
        ...
        match crate::zero_day::hpp::HppDetector::scan(
            ...
            Ok(hpp_summary) => {
                println!("  {} HPP results: {} vulnerable URLs, {} anomalies",
                    tc("✓", HISUI),
                    tc(&hpp_summary.vulnerable_count.to_string(), HISUI),
                    tc(&hpp_summary.total_anomalies.to_string(), HISUI));
            }
            Err(e) => {
        ...
        println!("  {} Anomalies detected: {}",
            tc("▸", GIN),
            tc(&total_anom.to_string(), HISUI));
        println!("  {} Exploits confirmed: {}",
            tc("▸", GIN),
            tc(&confirmed.to_string(), HISUI));

        let rate_color = if success_rate >= 50.0 { HISUI }
            else if success_rate >= 20.0 { HISUI }
            else { SHU };
...
fn truncate_url(url: &str, max: usize) -> String {
    if url.len() > max {
        let end = url.floor_char_boundary(max.saturating_sub(3));
        format!("{}...", &url[..end])
    } else {
        url.to_string()
    }
