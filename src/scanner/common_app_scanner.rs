...
use crate::cli::display::{HISUI, TSUYUKUSA, GIN, SHU};
...
impl CommonAppScanner {
    ...
    fn load_csv_tests(&mut self) {
        ...
        println!("{} Database found at: {}{}{}", tc("[DB-LOADER]", HISUI), BRIGHT_WHITE, enc_path.display(), RESET);
        ...
        match crate::db::decrypt_and_load(&enc_path) {
            Ok(rows) => {
                ...
                println!("{} Loaded {}{}{} tests from encrypted database", tc("[DB-LOADER]", HISUI), BRIGHT_WHITE, count, RESET);
            }
            Err(e) => {
    ...
    fn load_csv_tests_fallback(&mut self) {
        ...
        println!("{} Starting CSV database load...", tc("[DB-LOADER]", HISUI));
        ...
        for (idx, csv_file) in csv_files.iter().enumerate() {
            ...
            let progress = format!(
                "{} {}{}{} {} {}{}{} {} {}",
                tc(&format!("[{}]", frame), HISUI),
                DIM, "Loading", RESET,
                tc(&format!("[{}/{}]", current, total_files), TSUYUKUSA),
                BRIGHT_WHITE, "tests:", RESET,
                tc(&total_loaded.to_string(), HISUI),
                file_name
            );
        ...
        if loaded_files == total_files {
            println!("{} Successfully loaded {} CSV databases ({} tests)", tc("[DB-LOADER]", HISUI), tc(&format!("{}/{}", loaded_files, total_files), HISUI), total_loaded);
        } else {
            println!("{} Loaded {} CSV databases ({} tests)", tc("[DB-LOADER]", HISUI), tc(&format!("{}/{}", loaded_files, total_files), HISUI), total_loaded);
    ...
    pub async fn scan(&self, base_url: &str, download: bool) -> Vec<AppFinding> {
            ...
            if let Some((real_status, body)) = self.fetch_status_and_body(&url, test).await {
                ...
                if verdict.is_confirmed() {
                    ...
                    println!(
                        "\n  CGI   {}  {} HTTP {}  {}",
                        test.title,
                        tc(&format!("conf {:.0}%", verdict.confidence() * 100.0), GIN),
                        real_status,
                        url,
                    );
                    ...
                    if download && test.download_flag {
                        if let Some(ref dir) = download_dir {
                            match self.download_file(&url, test, dir).await {
                                Ok(path) => println!("      {} {}", tc("[SAVED]", HISUI), path.display()),
                                Err(e)   => eprintln!("      {} {}: {}", tc("[DL ERR]", SHU), url, e),
                            }
                        }
                    }
        ...
        println!("{}", final_line);
        println!("\n{} Scan complete -- {} confirmed / {} tests", tc("[CGI]", HISUI), hits, total);
        findings
    }
