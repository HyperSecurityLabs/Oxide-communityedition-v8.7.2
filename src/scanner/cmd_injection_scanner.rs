...
impl CmdInjectionScanner {
    ...
    pub async fn start_interactive_shell(&self, url: &str, param: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        println!("{}", format!("[+] Connected to {}@{}:{}",
            session.user.truecolor(TSUYUKUSA.0, TSUYUKUSA.1, TSUYUKUSA.2), url.truecolor(FUJI.0, FUJI.1, FUJI.2), session.working_directory.truecolor(HISUI.0, HISUI.1, HISUI.2)));
        ...
        loop {
            ...
            if command == "exit" || command == "quit" {
                println!("{}", "[+] Exiting shell session".truecolor(HISUI.0, HISUI.1, HISUI.2));
                break;
            }
            if command == "help" {
    ...
    fn show_shell_help(&self) {
        println!("{}", "\n─── Interactive Shell Help ───".truecolor(FUJI.0, FUJI.1, FUJI.2));
        ...
        println!("{}", "=============================".truecolor(FUJI.0, FUJI.1, FUJI.2));
    }
