...
use crate::cli::display::{FUJI, TSUYUKUSA};
            ...
            if let Ok(resp) = response {
                ...
                if acao == Some(origin) || acao == Some("*") {
                    findings.push(CorsFinding {
                        ...
                        description: format!("Server accepts dangerous origin scheme: {}", &origin[..origin.floor_char_boundary(origin.len().min(50))]),
                        evidence: format!("Origin: {} was allowed", &origin[..origin.floor_char_boundary(origin.len().min(50))]),
