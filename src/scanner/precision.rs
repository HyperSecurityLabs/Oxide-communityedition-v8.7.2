...
use crate::cli::display::{HISUI, TSUYUKUSA, GIN};
...
pub fn cgi_progress_line(
    ...
    let bar = format!(
        "{}{}",
        tc(&"=".repeat(filled), HISUI),
        tc(&"-".repeat(empty), GIN),
    );

    // Truncate path — plain ASCII ellipsis
    let path_display = if current_path.len() > 42 {
        format!("...{}", &current_path[current_path.floor_char_boundary(current_path.len().saturating_sub(39))..])
    } else {
    ...
    let hits_str = if hits > 0 {
        tc(&format!("[+{}]", hits), HISUI)
    } else {
        tc("[+0]", GIN)
    };

    format!(
        "\r{} CGI [{}] \x1B[97m{}/{}\x1B[0m {} {} {}",
        tc(spinner_frame, TSUYUKUSA),
        bar,
        current, total,
        tc(&format!("({}%)", pct), GIN),
        hits_str,
